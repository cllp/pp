define(['./module'], function (directives) {
    ...
    directives.directive('ppMultiSelect', function() {

        return {
            restrict: 'E',
            template: '<div class="form-group" ng-hide="hide">' +
                '<label ng-hide="{{label == null}}" class="control-label" for="{{id}}">{{label}}</label>' +
                '<select id="{{id}}" ng-model="model" ng-options="x.Name for x in optionsmodel track by x.Id" chosen data-placeholder="{{placeholder}}" ng-change="enableSave(true, item)" style="width: 100%" multiple>' +
                '<option value=""></option>' +
                '</select>' +
                '</div>',
            scope: {
